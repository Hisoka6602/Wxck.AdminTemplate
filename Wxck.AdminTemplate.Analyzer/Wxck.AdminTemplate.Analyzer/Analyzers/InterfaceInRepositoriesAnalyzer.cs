...
namespace Wxck.AdminTemplate.Analyzer.Analyzers {
    ...
    public class InterfaceInRepositoriesAnalyzer : DiagnosticAnalyzer {
        public const string DiagnosticId = "DOM001";

        private static readonly DiagnosticDescriptor Rule = new(
            id: DiagnosticId,
            title: "Repositories 目录下只能定义接口",
            messageFormat: "类 '{0}' 不允许在 Repositories 目录下定义，必须是接口。",
            category: "项目结构",
        ...
        private void AnalyzeSymbolDefinition(SymbolAnalysisContext context) {
            ...
            if (symbol is INamedTypeSymbol namedTypeSymbol) {
                ...
                // 判断文件是否位于 Domain 项目的 Repositories 目录下
                if (filePath.Contains("Domain") && filePath.Contains("Repositories")) {
                    // 如果是类类型而不是接口，则触发诊断
