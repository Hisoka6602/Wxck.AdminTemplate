...
namespace Wxck.AdminTemplate.ServiceDefaults {
    ...
    public static class ServiceCollectionExtensions {
        ...
        public static IServiceCollection AddRepositories(this IServiceCollection services) {
            var assemblies = LoadInfrastructureAssemblies();
        ...
        private static List<Assembly> LoadInfrastructureAssemblies() {
            var loadedAssemblies = new List<Assembly>();
            var basePath = AppContext.BaseDirectory;

            var dllFiles = Directory.GetFiles(basePath, "*.dll")
                .Where(f => Path.GetFileName(f).Contains("Infrastructure", StringComparison.OrdinalIgnoreCase));
